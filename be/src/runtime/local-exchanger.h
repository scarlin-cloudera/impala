...
/// consumers. Each consumer gets its own copy of each RowBatch and tracks its own memory
/// ownership. The last consumer to pull a RowBatch takes ownership of the original
/// RowBatch. Other consumers get a copy. Thread-safe. Inspired by StarRocks'
...
class LocalExchanger {
 public:
  ...
  void Close(int32_t consumer_index);

  /// Returns true if all consumers have closed.
