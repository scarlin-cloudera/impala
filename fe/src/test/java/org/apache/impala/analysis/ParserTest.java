...
public class ParserTest extends FrontendTestBase {
  ...
  public void TestUuidTypes() {
    // Scalar UUID in DDL, CAST, and the generic complex-type shapes
    // exercised by TypeDefsParseOk().
    TypeDefsParseOk("UUID");

    // Additional nested/complex shapes not generated by TypeDefsParseOk().
    TypeDefsParseOk("MAP<UUID, STRING>");
    TypeDefsParseOk("MAP<STRING, UUID>");
    TypeDefsParseOk("STRUCT<id:UUID, name:STRING>");
    TypeDefsParseOk("STRUCT<data:STRING, uuid:UUID COMMENT 'identifier'>");
    TypeDefsParseOk("ARRAY<STRUCT<id:UUID, val:INT>>");
    TypeDefsParseOk("MAP<UUID, ARRAY<STRING>>");
    TypeDefsParseOk("STRUCT<ids:ARRAY<UUID>>");

    // Full CREATE TABLE statements with Iceberg storage.
    ParsesOk("CREATE TABLE t (id UUID, name STRING) STORED BY ICEBERG");
    ParsesOk("CREATE TABLE t (s STRUCT<uuid_col:UUID, val:INT>) STORED BY ICEBERG");
  }
