 ...
 * count needed to handle the delete files. All other non-count(*) expressions remain
 * as/is (if they exist). So if the output of the Agg was 2 columns, sum(col) and
 * count(*), the 2 columns of the project will be "InputRef($0)" (for the sum) and
 * "InputRef($1) + <precalculated count(*) constant>" for the count(*).
 *
 ...
 protected static final Logger LOG = LoggerFactory.getLogger(IcebergCountStarOptimizer.class.getName());
  ...
  private RelNode transformForV2CountStarOptimization(LogicalAggregate agg,
    ...
    for (AggregateCall aggCall : agg.getAggCallList()) {
      ...
      if (!(aggCall.getAggregation().getKind().equals(SqlKind.COUNT) &&
        ...
        // For any non-count(*), the input is just the inputref, which is the value that
        // comes out of the agg.
        projects.add(aggInput);
      } else {
        // For the count(*), we add the input ref to the count literal constant from the v2 count.
  ...
  private RelNode transformForV1CountStarOptimization(LogicalAggregate agg,
    ...
    if (hasCountStarOnly(agg)) {
      ...
      return LogicalValues.create(cluster, agg.getRowType(), ImmutableList.of(ImmutableList.copyOf(literals)));
    } else {
