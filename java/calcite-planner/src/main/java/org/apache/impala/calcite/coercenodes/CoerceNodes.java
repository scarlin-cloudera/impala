...
public class CoerceNodes{
  ...
  private static List<RelDataType> getCastedOperandTypes(AggregateCall aggCall,
    ...
    RelDataType retType = fn.getReturnType().isWildcardDecimal()
        ? CoerceOperandShuttle.getCommonDecimalRelDataType(aggCall.getAggregation(),
            operandTypes, factory)
    ...
    for (int i = 0; i < operandTypes.size(); ++i) {
      ...
      // special case for functions like NDV.  If the decimal function is to be used,
      // the operand should already be a non-wildcard decimal.
