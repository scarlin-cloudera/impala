...
public class CalciteRelNodeConverter implements CompilerStep {
  ...
  public CalciteRelNodeConverter(CalciteAnalysisResult analysisResult) {
    ...
    this.planner_ = new VolcanoPlanner(ImpalaCost.FACTORY, new ImpalaLoptOptimizeExtension.RuntimeFilterInfo(analysisResult.getAnalyzer().getQueryOptions()));
  ...
  public CalciteRelNodeConverter(CalciteValidator validator) {
    ...
    this.planner_ = new VolcanoPlanner(ImpalaCost.FACTORY, new ImpalaLoptOptimizeExtension.RuntimeFilterInfo(null));
    planner_.addRelTraitDef(ConventionTraitDef.INSTANCE);
    planner_.setExecutor(new RemoveUnraggedCharCastRexExecutor());
    cluster_ =
