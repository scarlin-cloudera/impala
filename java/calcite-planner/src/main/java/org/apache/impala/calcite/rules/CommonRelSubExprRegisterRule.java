...
import org.apache.impala.calcite.cte.Registry;
...
/**
 * Rule for saving relational expressions that appear more than once in a query tree to
 * the planner context.
 *
 * Note that Calcite matches based on the CommonRelSubExprRule. Skipping that inheritance
 * results in very different behavior.
 */
@Value.Enclosing
public final class CommonRelSubExprRegisterRule extends CommonRelSubExprRule {
  public static final CommonRelSubExprRegisterRule JOIN =
      new CommonRelSubExprRegisterRule(Config.JOIN);
  public static final CommonRelSubExprRegisterRule AGGREGATE =
      new CommonRelSubExprRegisterRule(Config.AGGREGATE);
  public static final CommonRelSubExprRegisterRule FILTER =
      new CommonRelSubExprRegisterRule(Config.FILTER);
  public static final CommonRelSubExprRegisterRule PROJECT =
      new CommonRelSubExprRegisterRule(Config.PROJECT);

  protected CommonRelSubExprRegisterRule(Config config) {
    super(config);
  }

  @Override
  public void onMatch(final RelOptRuleCall call) {
    Registry r = call.getPlanner().getContext().unwrap(Registry.class);
    r.add(call.rel(0));
  }
   ...
   *
   * The notion of interesting is loosely defined on purpose since it may change as the
   * API evolves. At the moment an expression is considered interesting if it contains at
   * least one of the following RelNode types:
   * <ul>
   *   <li>{@link Join}</li>
  ...
  public interface Config extends CommonRelSubExprRule.Config {
    Config JOIN = ImmutableCommonRelSubExprRegisterRule.Config.builder()
        .operandSupplier(o -> o.operand(Join.class).predicate(
            j -> JoinRelType.INNER == j.getJoinType()).anyInputs()).build();
    Config AGGREGATE = ImmutableCommonRelSubExprRegisterRule.Config.builder()
        .operandSupplier(o -> o.operand(Aggregate.class).anyInputs()).build();
    Config FILTER = ImmutableCommonRelSubExprRegisterRule.Config.builder()
        .operandSupplier(o -> o.operand(Filter.class).anyInputs()).build();
    Config PROJECT = ImmutableCommonRelSubExprRegisterRule.Config.builder()
        .operandSupplier(o -> o.operand(Project.class).predicate(
            new InterestingRelNodePredicate()).anyInputs()).build();
