...
@Value.Enclosing
public class CountStarIcebergRule
    extends RelRule<CountStarIcebergRule.Config>
    implements TransformationRule {
 protected static final Logger LOG = LoggerFactory.getLogger(CountStarIcebergRule.class.getName());

 public static final CountStarIcebergRule INSTANCE =
      CountStarIcebergRule.Config.DEFAULT.toRule();

  protected CountStarIcebergRule(Config config) {
    super(config);
  }
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT = ImmutableCountStarIcebergRule.Config.of("dummy")
        .withOperandSupplier(b0 ->
            b0.operand(Aggregate.class).oneInput(b1 ->
                b1.operand(TableScan.class)
                    .predicate(CountStarIcebergRule::test).noInputs()));

    @Value.Parameter
    //XXX: need to figure out how to generate of() without this
    public abstract String dummy();

    @Override default CountStarIcebergRule toRule() {
      return new CountStarIcebergRule(this);
    }
  }
}
