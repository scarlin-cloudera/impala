...
public class ImpalaCTEProducer extends Spool implements ImpalaPlanRel {
  ...
  private final int numConsumers_;

  public ImpalaCTEProducer(RelNode cte, String name, int numConsumers) {
    this(cte.getCluster(), cte.getTraitSet(), cte, name, numConsumers, Type.EAGER,
        Type.EAGER);
  }

  private ImpalaCTEProducer(RelOptCluster cluster, RelTraitSet traitSet, RelNode cte,
      String name, int numConsumers, Type readType, Type writeType) {
  ...
  protected ImpalaCTEProducer copy(
    ...
    return new ImpalaCTEProducer(
        getCluster(), traitSet, input, name_, numConsumers_, readType, writeType);
  }
  ...
  public NodeWithExprs getPlanNode(ParentPlanRelContext context) throws ImpalaException {
    ...
    PlanNode physicalNode =
        new CTEProducerNode(nodeId, child.planNode_, name_, numConsumers_);
    physicalNode.init(context.ctx_.getRootAnalyzer());
    return new NodeWithExprs(
        physicalNode, child.outputExprs_, getRowType().getFieldNames());
  }
